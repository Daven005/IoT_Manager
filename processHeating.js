...
exports.firingReason = function (onoff, temperature, callback) {
  ...
  if (onoff == 'on') {
    // console.log(`info: ${JSON.stringify(zones)}`);
    info.demand = zones.filter((z) => (z && z.demand && !z.IsMaster)).map((z) => {
      return {
        'name': z.Name,
...
function checkNewZoneHeat(z, timeNow, dateStr, dow, prevDow, nextDow) {
  ...
  return {
    next: programme[pIdx - 1].temperature,
